                ...
                <tbody>
                  {patients.map((p, idx) => (
                    <tr key={p._id || idx}>
                      ...
                      <td>{p.age}</td>
                      <td>
                        <span className={`v2-badge-risk v2-badge-risk--${(p.niveauRisque || "Faible").toLowerCase()}`}>
                          {p.niveauRisque || "Faible"}
                        </span>
                      </td>
                      <td>
                        {p.derniereAnalyseIA 
                          ? new Date(p.derniereAnalyseIA).toLocaleDateString() 
                          : "Aucun"}
                      </td>
                      <td>
                        <span className={`v2-status-pill v2-status--${(p.etat || "Stable").toLowerCase()}`}>
                          {p.etat}
                        </span>
                      </td>
                      <td>
                        <Link
                          to={`/cardiologue/patients/${p._id}`}
